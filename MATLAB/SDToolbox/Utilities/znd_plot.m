...
%
%         xunits:      distance units in plot axis label, default 'm'
%
%         xmult        multiplier for distance units,  default 1.0
%
...
%     Most recent edit JES 4/30/2026
    ...
    default_species_min = 1E-8;
    default_fig_type = 'pdf';
    %
    p = inputParser;
    ...
    addParameter(p,'fig_type',default_fig_type);
    addParameter(p,'species_min',default_species_min);
    %
    ...
    fig_type = p.Results.fig_type;
    species_min = p.Results.species_min;
    %
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    if maxx == 0
        [~,k] = max(znd_output.T);
    ...
    figT = figure('Visible',visible);
 	plot(xmult*znd_output.distance(:),znd_output.T(:),'k');
   	xlabel(x_label,'FontSize',fontsize,"Interpreter","latex");
 	ylabel('$T$ [K]','FontSize',fontsize,"Interpreter","latex");
    xlim([minx maxx]);
 	set(gca,'FontSize',12,'XScale',xscale);
    if (prnfig) exportgraphics(figT,['znd_temperature','.',fig_type]); end
	
	% Pressure as a function of position
    figP = figure('Visible',visible);
	plot(xmult*znd_output.distance(:),znd_output.P(:),'k');
 	xlabel(x_label,'FontSize',fontsize,"Interpreter","latex");
	ylabel('$P$ [Pa]','FontSize',fontsize,"Interpreter","latex");
	xlim([minx maxx]);
    set(gca,'FontSize',12,'XScale',xscale);
    if (prnfig) exportgraphics(figP,['znd_pressure','.',fig_type]); end
    
    % Mach number as a function of position
    figM = figure('Visible',visible);
    plot(xmult*znd_output.distance(:),znd_output.M(:),'k');
 	xlabel(x_label,'FontSize',fontsize,"Interpreter","latex");
    ylabel('$M$','FontSize',fontsize,"Interpreter","latex");
    xlim([minx maxx]);
    set(gca,'FontSize',12,'XScale',xscale);
    if (prnfig) exportgraphics(figM,['znd_Mach','.',fig_type]); end
    
    % Thermicity as a function of position
    figS = figure('Visible',visible);
    plot(xmult*znd_output.distance(:),znd_output.thermicity(:),'k');
 	xlabel(x_label,'FontSize',fontsize,"Interpreter","latex");
    ylabel('$\dot{\sigma}$ [s$^{-1}$]','FontSize',fontsize,"Interpreter","latex");
    xlim([minx maxx]);
    set(gca,'FontSize',12,'XScale',xscale);
    if (prnfig) exportgraphics(figS,['znd_thermicity','.',fig_type]); end
    ...
    if ~isempty(major_species)
        ...
        if ~isempty(major_k)
            ...
            if strcmp(string(species_plt),'mass')
                ...
                ylabel('$Y_i$','FontSize',fontsize,"Interpreter","latex");
            elseif strcmp(string(species_plt),'mole')
                plot(xmult*znd_output.distance(:),znd_output.speciesX(:,major_k));
                ylabel('$X_i$','FontSize',fontsize,"Interpreter","latex");
            end
            axis([minx maxx species_min 1]);
           	xlabel(x_label,'FontSize',fontsize,"Interpreter","latex");
            legend(major_labels,'Location','southeast');
            if (prnfig) exportgraphics(figMaj,['znd_major_species','.',fig_type]); end
            figs(end+1) = figMaj;
        end
    end
    ...
    if ~isempty(minor_species)
        ...
        if ~isempty(minor_k)
            ...
            if (prnfig) exportgraphics(figMin,['znd_minor_species','.',fig_type]); end
            figs(end+1) = figMin;
        end
    end
