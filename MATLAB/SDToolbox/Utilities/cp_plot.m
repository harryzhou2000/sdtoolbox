...
%                       default is 'mole"
%         
%       P_scale =  scaling factor for pressure, default = 1  (0 to skip
%       plot)
%
...
%       as part of output.  
%
%        fig_type = MATLAB figure type specification, default -dpdf       
%         
...
%     Most recent edit:  JES 6/22/2025
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    default_t_unit = '$t$ [s]';
    default_P_unit ='$P$ [Pa]';
    default_rho_unit = '$\rho$ [kg$\cdot$m$^{-3}$]';
    default_prnfig = false;
    default_fig_type = '-dpdf';
    ...
    if (P_scale ~= 0)
        ...
        xlabel(t_unit,'FontSize',fontsize,'Interpreter','latex');
        ylabel(P_unit,'FontSize',fontsize,'Interpreter','latex');
        xlim([mint/t_scale maxt/t_scale]);
        set(gca,'FontSize',12,'XScale',xscale);
        if (prnfig) print('cp_pressure',fig_type); end
    end
	...
	xlabel(t_unit,'FontSize',fontsize,'Interpreter','latex');
	ylabel(rho_unit,'FontSize',fontsize,'Interpreter','latex');
	xlim([mint/t_scale maxt/t_scale]);
    set(gca,'FontSize',12,'XScale',xscale);
    if (prnfig) print('cp_density',fig_type); end
    ...
    if ~isempty(major_species)
        ...
        if ~isempty(major_k)
            ...
            if (prnfig) print('cp_major_species',fig_type); end
            figs(end+1) = figMaj;
        end
    end
    ...
    if ~isempty(minor_species)
        ...
        if ~isempty(minor_k)
            ...
            if (prnfig) print('cp_minor_species',fig_type); end
            figs(end+1) = figMin;
        end
    end 
