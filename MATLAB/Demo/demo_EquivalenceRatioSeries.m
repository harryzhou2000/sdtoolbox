...
for i = 1:npoints
    ...
    phi(i) = phimin + (i-1)*deltaphi;
    x = zeros(nsp, 1);
    xfuel(i) = stoich*phi(i)/(1.0 + beta +stoich*phi(i));
    xoxidizer(i) = 1.0/(1.0+ beta + stoich*phi(i));
    x(ifuel,1) = xfuel(i);
    x(ioxidizer,1) = xoxidizer(i);
    x(idiluent,1) = beta/(1.0+ beta + stoich*phi(i));
    disp(['Case ', num2str(i), ': phi = ', num2str(phi(i))]);
    disp('Molefractions');
...
if (output)
    ...
    fprintf(fid, ['Variables = "Equivalence Ratio","X fuel","X oxidizer","CV Pressure","CV Temperature",' ...
    ...
    z = [phi; xfuel; xoxidizer; uv_P; uv_T; hp_exp; hp_ae; Ucj; cj_P; cj_af; vn_P; ref_P; Uref; P3; ind_len_ZND;...
        exo_len_ZND; theta_effective_CV];
    fprintf(fid, '%14.5e \t %14.5e \t %14.5e \t %14.5e \t %14.5e \t %14.5e \t %14.5e \t %14.5e \t %14.5e  \t %14.5e  \t %14.5e  \t %14.5e  \t %14.5e  \t %14.5e \t %14.5e \t %14.5e \t %14.5e\n', z);
    fclose(fid);
end
...
if (plots)
    ...
    plot(phi(:),cj_T(:),'r',phi(:),hp_T(:),'g',phi(:),uv_T(:),'b');
    legend('CJ', 'HP', 'UV');
    hold off;
    ...
    set(gca,'FontSize',fontsize,'LineWidth',2);
    plot(phi(:),ref_P(:)/1E6,'r',phi(:),cj_P(:)/1E6,'b',phi(:),uv_P(:)/1E6,'g',phi(:),P3(:)/1E6,'c');
    ...
    axis([minx maxx miny maxy])
    set(gca,'FontSize',fontsize,'LineWidth',2);
    plot(phi(:),Uref(:),'r',phi(:),Ucj(:),'b',phi(:),hp_ae(:),'g');
    ...
    set(gca,'FontSize',fontsize,'LineWidth',2);
    semilogy(phi(:),ind_len_ZND(:),'r',phi(:),exo_len_ZND(:),'b');
    axis([minx maxx miny maxy])
