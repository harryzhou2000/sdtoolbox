...
% Compute .yaml file coefficients, adjust values to compensate for average
% molar mass, and matching HP temperature and CJ speed.  Test one-step and
% two-gamma .cti file models
%
% Requires creating appropriate .yaml files and setting case switch
% 'yaml_file' to treat case of interest, editing species list to suit. 
% This version is set to use stoichiometric pentane-air as the reactants
% and example .yaml files are given on the SDT website
%  
% ################################################################################
...
switch yaml_file
    ...
    case 'two-gamma_lo'
        %low temperature fit
        mech = 'pentane_two_gamma.yaml';
        gas  = Solution(mech);  % constant pressure thermo
        q = 'A:1.0 B:0.0';
        plot_species={'A','B'}; mech = 'pentane_two_gamma.yaml';
        curve_fit = false;
    case 'two-gamma_hi'
        % high temperature fit
        mech = 'pentane_two_gamma_CJ.yaml';
