...
if output:
    ...
    fid.write('Variables = "Equivalence Ratio","X fuel","X oxidizer","X diluent","CV Pressure","CV Temperature","HP expansion","HP soundspeed","CJ Speed","CJ Pressure ","CJ sound speed","vN Pressure ","Reflected Pressure","Reflected Speed","Plateau Pressure","Ind Length","Energy Length","Theta Effective"\n')
    for i in range(npoints): 
        fid.write('%14.5e \t %14.5e \t %14.5e \t %14.5e \t %14.5e \t %14.5e \t %14.5e \t %14.5e \t %14.5e \t %14.5e  \t %14.5e  \t %14.5e  \t %14.5e  \t %14.5e  \t %14.5e \t %14.5e \t %14.5e \t %14.5e\n' %
                  (phi[i],xfuel[i],xoxidizer[i],xdiluent[i],uv_P[i],uv_T[i],hp_exp[i],hp_ae[i],Ucj[i],cj_P[i],cj_af[i],vn_P[i],ref_P[i],Uref[i],P3[i],ind_len_ZND[i],exo_len_ZND[i],theta_effective_CV[i]))
    fid.close()


if plots:
    xfuelp = 100*np.asarray(xfuel)
    plt.figure(1)
    ...
    minx = min(xfuelp)
    fontsize=12
    plt.xlabel('Fuel fraction (%)',fontsize=fontsize)
    plt.ylabel('Temperature (K)',fontsize=fontsize)
    plt.title('Explosion Temperature',fontsize=fontsize)
    plt.axis([minx,maxx,miny,maxy])
    plt.plot(xfuelp,cj_T,'r')
    plt.plot(xfuelp,hp_T,'g')
    plt.plot(xfuelp,uv_T,'b')
    plt.legend(['CJ', 'HP', 'UV'])
    plt.tight_layout()
    ...
    plt.plot(xfuelp,ref_P/1e6,'r')
    plt.plot(xfuelp,cj_P/1e6,'b')
    plt.plot(xfuelp,uv_P/1e6,'g')
    plt.plot(xfuelp,P3/1e6,'c')
    ...
    plt.axis([minx,maxx,miny,maxy])
    plt.plot(xfuelp,Uref,'r')
    plt.plot(xfuelp,Ucj,'b')
    plt.plot(xfuelp,hp_ae,'g')
    ...
    plt.axis([minx,maxx,miny,maxy])
    plt.semilogy(xfuelp,ind_len_ZND,'r')
    plt.semilogy(xfuelp,exo_len_ZND,'b')
